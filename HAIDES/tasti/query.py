...
def DNNcache(target_dnn_cache, score_fn, idx):
    value = target_dnn_cache[idx]
    value = score_fn(value)
    return float(value)
...
class BaseQuery:
    ...
    def propagate(self, target_dnn_cache, reps, topk_reps, topk_distances):
        ...
        if not self.df:
            ...
            y_true_metrics = np.array(
                [DNNcache(target_dnn_cache, score_fn, idx) for idx in range(len(topk_reps))]
            )

            y_pred = np.zeros(len(topk_reps))

            print("frames for which predicate is TRUE", np.count_nonzero(y_true_metrics))

            for i in tqdm(range(len(y_pred)), 'Propagation'):

                if(len(y_true[topk_reps[i]]) > 1):
        ...
        precision, recall, fscore, support = sklearn.metrics.precision_recall_fscore_support(np.rint(y_true_metrics).astype(int), np.rint(y_pred).astype(int))
