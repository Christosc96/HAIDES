...
class FPFRandomExtendingBucketter(Bucketter):
    def bucket(
            self,
            embeddings: np.ndarray,
            max_k: int,
        ...
        buckets_per_query = 10000
        print(buckets_per_query)
        reps_added = 0
        ...
        for i in tqdm.tqdm(range(num_random, self.nb_buckets), desc='FPFBucketter'):
            ...
            reps_added +=1
            if(reps_added == buckets_per_query):
                reps_added = 0
                buckets_per_query = 5000
                print(max_k)
                transposed_dists = dists.transpose()
                topk_reps = self.topk(max_k, transposed_dists)
                topk_dists = np.zeros([len(topk_reps), max_k])
        
                for i in range(len(topk_dists)):
                    topk_dists[i] = transposed_dists[i, topk_reps[i]]
                for i in range(len(topk_reps)):
                    topk_reps[i] = reps[topk_reps[i]]
