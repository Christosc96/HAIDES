...
ROOT_DATA_DIR = '/work/823656/data/datasets/nightstreet'
...
class NightStreetQueryPredicate1(tasti.BaseQuery):
    def score(self, target_dnn_output):
        return 1.0 if len(target_dnn_output) == 1 else 0.0

class NightStreetQueryPredicate2(tasti.BaseQuery):
    def score(self, target_dnn_output):
        return 1.0 if len(target_dnn_output) == 2 else 0.0

class NightStreetQueryPredicate3(tasti.BaseQuery):
    def score(self, target_dnn_output):
        return 1.0 if len(target_dnn_output) == 3 else 0.0

class NightStreetQueryPredicate4(tasti.BaseQuery):
    def score(self, target_dnn_output):
...
if __name__ == '__main__':
    ...
    #print("RUNNING WITH RANDOM")
    #config.do_bucketting = True
    print("RUNNING WITH BRANCHING FACTOR K = ", config.nb_buckets)
    index = NightStreetOfflineIndex(config)
    index.init()

    print("QUERY CARS = 1")
    for i in [10000, 13000, 15000, 17000, 20000]:
        index = NightStreetOfflineIndex(config)
        config.max_oracle_calls = i
        index.init()
        query = NightStreetQueryPredicate1(index)
    ...
    for i in [10000, 13000, 15000, 17000, 20000]:
        ...
        config.max_oracle_calls = i
        index.init()
        query = NightStreetQueryPredicate2(index)
        index.adapt_to_predicate(query.score, config.max_oracle_calls, verbose=0)
        query.execute()

'''

    original_embeddings = index.embeddings
    init_size = len(index.embeddings)

    print("scalability test, memory (at 1% budget of the total dataset)")
    ...
    for i in [0.25, 0.50, 0.75, 1.00]:
        ...
        index.embeddings = rng.choice(original_embeddings, int(i*init_size), replace=False)
        print(len(index.embeddings))
        #config.nb_buckets = int(0.01*len(index.embeddings))
        index.do_bucketting()
'''
